            ...
            <section className="relative overflow-hidden pt-40 pb-40 min-h-[90vh] flex items-center">
                ...
                <div className="container mx-auto px-6 flex flex-col items-center text-center relative z-10">
                    ...
                    <p className="text-white/60 text-lg md:text-xl max-w-2xl leading-relaxed mb-16 font-medium">
                        {t('우리는 움직임 하나가 인간 삶의 근원적인 감각임을 탐구하며, 골격 정렬 속에 숨겨진 물리적 원리를 규명합니다.')}
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        <Link to="/archive" className="px-10 py-5 premium-gradient rounded-full font-bold text-lg shadow-2xl shadow-primary/40 hover:scale-105 transition-all flex items-center gap-3">
                            {t('연구 자료실 접속하기')} <ArrowRight size={20} />
                        </Link>
            ...
            <section id="philosophy" className="py-40 bg-[#020510]/30 backdrop-blur-sm">
                <div className="container mx-auto px-6">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
                        <div>
                            <div className="text-primary text-[10px] font-black tracking-widest uppercase mb-4">{t('학문 중심 기조')}</div>
                            <h2 className="text-4xl md:text-6xl font-black tracking-tighter">{t('동력 과학의 근간')}</h2>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="group glass-effect p-10 rounded-[40px] hover:border-primary/50 transition-all">
                            ...
                            <h3 className="text-2xl font-bold mb-6">{t('운동 사슬 (Kinetic Chains)')}</h3>
                            <p className="text-white/50 leading-relaxed mb-10">{t('척추 분절의 유기적 조절과 신체적 움직임을 통해 각 관절의 상호 협조성을 극대화합니다.')}</p>
                            <Link to={archiveLink} className="text-xs font-black tracking-widest uppercase flex items-center gap-2 group-hover:text-primary transition-colors">{t('데이터 탐독')} <ArrowRight size={14} /></Link>
                        </div>
                        ...
                        <div className="group glass-effect p-10 rounded-[40px] hover:border-primary/50 transition-all">
                            ...
                            <h3 className="text-2xl font-bold mb-6">{t('무게 중심 (Center of Gravity)')}</h3>
                            <p className="text-white/50 leading-relaxed mb-10">{t('중력 균형의 유지 관점에서 신체적 상호작용 및 고유 감각을 최상의 상태로 정렬합니다.')}</p>
                            <Link to={archiveLink} className="text-xs font-black tracking-widest uppercase flex items-center gap-2 group-hover:text-primary transition-colors">{t('데이터 탐독')} <ArrowRight size={14} /></Link>
                        </div>
                        ...
                        <div className="group glass-effect p-10 rounded-[40px] hover:border-primary/50 transition-all">
                            ...
                            <h3 className="text-2xl font-bold mb-6">{t('구조적 통합 (Structural Integrity)')}</h3>
                            <p className="text-white/50 leading-relaxed mb-10">{t('습관적 정렬의 불균형을 해결하여 개개인이 가진 본연의 신체적 잠재력을 끌어올립니다.')}</p>
                            <Link to={archiveLink} className="text-xs font-black tracking-widest uppercase flex items-center gap-2 group-hover:text-primary transition-colors">{t('데이터 탐독')} <ArrowRight size={14} /></Link>
                        </div>
                    </div>
                </div>
            ...
            <section className="py-40">
                <div className="container mx-auto px-6">
                    <div className="relative overflow-hidden p-16 md:p-24 rounded-[60px] glass-effect text-center">
                        ...
                        <h2 className="text-4xl md:text-7xl font-black mb-10 tracking-tighter">{t('연구회원 가입하기')}</h2>
                        <p className="text-white/60 text-lg md:text-xl max-w-2xl mx-auto mb-16 leading-relaxed">
                            {t('분과 전문가 활동가, 생체학적 연구자들과 함께 하십시오. 실전 경험, 영상 자료, 그리고 전문 트레이닝 가이드를 모두 제공합니다.')}
                        </p>
