    ...
    if (loading) {
        return (
            <div className="pt-40 pb-20 min-h-screen flex items-center justify-center">
                <div className="text-white/30 font-black animate-pulse tracking-widest uppercase">Streaming Reference Data...</div>
            </div>
        )
    }
        ...
        return (
            <div className="pt-40 pb-20 min-h-screen container mx-auto px-6">
                <div className="relative overflow-hidden p-20 md:p-32 rounded-[60px] glass-effect text-center flex flex-col items-center">
                    ...
                    <h2 className="text-4xl md:text-6xl font-black mb-10 tracking-tighter">프리미엄 전용 영상</h2>
                    <p className="text-white/60 text-lg md:text-xl max-w-2xl leading-relaxed mb-16 font-medium">
                        이 참고 영상 라이브러리는 멤버십 전용입니다. <br />
                        고해상도 분석 영상과 전문 시술 테크닉을 확인하세요.
                    </p>
    ...
    return (
        <div className="pt-40 pb-20 min-h-screen">
            <div className="container mx-auto px-6">
                <div className="mb-20">
                    <div className="text-primary text-[10px] font-black tracking-widest uppercase mb-4">Visual Library</div>
                    <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-6">참고 영상</h1>
                    <p className="text-white/50 text-lg font-medium">동력 과학 이론의 실제 적용 사례를 고화질 영상으로 분석합니다.</p>
                </div>
                ...
                {contents.length === 0 ? (
                    <div className="py-40 text-center glass-effect rounded-[40px] border border-dashed border-white/10">
                        ...
                        <p className="text-white/30 font-medium">등록된 참고 영상이 아직 없습니다.</p>
                    </div>
                ) : (
                    <>
                            ...
                            {paginatedContents.map((content) => (
                                <div key={content.id} className="group glass-effect p-10 rounded-[40px] hover:border-primary/50 transition-all flex flex-col h-full border border-white/5">
                                    ...
                                    <Link to={`/contents/${content.id}`} className="mt-12 flex items-center justify-between group/btn">
                                        <span className="text-xs font-black tracking-widest uppercase text-white/30">Watch Video</span>
