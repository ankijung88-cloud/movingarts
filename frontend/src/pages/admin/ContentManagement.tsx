    ...
    return (
        <div className="container mx-auto">
            <div className="flex justify-between items-end mb-12">
                <div>
                    <h2 className="text-4xl font-black tracking-tighter mb-4">콘텐츠 관리</h2>
                    <p className="text-white/50 text-sm">학술 자료 및 연구 정보를 직접 등록하고 관리합니다.</p>
                </div>
                <button
            ...
            {isFormOpen && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-[#050A1A]/90 backdrop-blur-xl">
                    <div className="glass-effect w-full max-w-2xl p-12 rounded-[40px] relative">
                        <h3 className="text-2xl font-black mb-10 tracking-tighter">{editMode ? '콘텐츠 수정' : '새 콘텐츠 등록'}</h3>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black tracking-widest uppercase text-white/40">제목</label>
                                <input
                                    type="text" placeholder="연구 주제를 입력하세요..." required
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black tracking-widest uppercase text-white/40">내용</label>
                                <textarea
                                    placeholder="상세 내용을 입력하세요..." required rows={6}
                            ...
                            <div className="flex gap-4 pt-6">
                                <button type="submit" className="flex-grow py-5 premium-gradient rounded-full font-bold text-lg">저장하기</button>
                                <button type="button" onClick={() => setIsFormOpen(false)} className="px-10 py-5 glass-effect rounded-full font-bold text-lg">취소</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {contents.map((content) => (
                    <div key={content.id} className="glass-effect p-8 rounded-[32px] group hover:border-primary/50 transition-all border border-white/5">
                        ...
                        <h4 className="text-xl font-bold mb-4 line-clamp-2">{content.title}</h4>
                        <p className="text-white/40 text-sm mb-6 line-clamp-3 leading-relaxed">{content.content}</p>
            ...
            {loading && <div className="py-20 text-center text-white/30">학술 데이터를 동기화하는 중...</div>}
            {!loading && contents.length === 0 && !isFormOpen && (
                <div className="py-40 text-center glass-effect rounded-[40px] border border-dashed border-white/10">
                    ...
                    <p className="text-white/30 font-medium">등록된 연구 콘텐츠가 없습니다.</p>
                </div>
            )}
        </div>
    )
