...
const Checkout = () => {
    ...
    const planName = plan === 'monthly' ? '월간 연구 멤버십' : '연간 연구 멤버십'
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            alert('구독 결제가 완료되었습니다! 연구소 서비스를 이용하실 수 있습니다.')
            navigate('/')
        } catch (err) {
            ...
            <div className="container mx-auto px-6 max-w-6xl">
                <div className="flex flex-col lg:flex-row gap-16">
                    ...
                    <div className="flex-grow">
                        <h1 className="text-4xl font-bold mb-10 tracking-tight">구독 결제</h1>
                        <p className="text-white/50 mb-12">연구 정보 구독을 위해 결제 정보를 입력해주세요.</p>

                        <div className="space-y-12">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black tracking-widest uppercase text-white/40">가입 이메일</label>
                                    <input
                                        type="email"
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black tracking-widest uppercase text-white/40">신청인 이름</label>
                                    <input
                                        type="text"
                                        placeholder="홍길동"
                                        className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:border-primary outline-none transition-colors"
                            ...
                            <div className="space-y-6">
                                <label className="text-[10px] font-black tracking-widest uppercase text-white/40">결제 수단 선택</label>
                                <div className="grid grid-cols-3 gap-4">
                                    ...
                                    <button className="flex flex-col items-center gap-3 py-6 rounded-2xl bg-white/5 border border-white/10 text-white/40 hover:border-white/20 transition-all font-bold">
                                        <Apple size={24} />
                                        <span className="text-xs">애플페이</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-primary/10 border border-primary/20 rounded-[32px] p-8 flex gap-6 items-start">
                                ...
                                <div>
                                    <h4 className="font-bold mb-2">보안 결제 안내</h4>
                                    <p className="text-sm text-white/50 leading-relaxed">
                                        본 연구소는 포트원(PortOne) 보안 시스템을 통해 귀하의 결제 정보를 암호화 기술로 보호합니다.
                                        신용카드 번호는 서버에 직접 저장되지 않으며 결제사로 즉시 전달됩니다.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="w-full lg:w-[400px]">
                        <div className="glass-effect rounded-[40px] p-10 sticky top-32">
                            ...
                            <div className="mb-10">
                                ...
                                <div className="text-white/40 text-xs">최종 승인일로부터 즉시 멤버십 권한이 부여됩니다.</div>
                            </div>

                            <div className="space-y-6 mb-12 border-y border-white/5 py-8">
                                <div className="flex justify-between text-sm">
                                    <span className="text-white/40">기본 금액</span>
                                    <span>₩{amount.toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-white/40">할인 금액</span>
                            ...
                            >
                                {isLoading ? '처리 중...' : `₩${amount.toLocaleString()} 결제하기`}
                            </button>
